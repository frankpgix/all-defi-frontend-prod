...
const Connect: FC<Props> = ({ simple, children, as = 'div' }) => {
  ...
  const { hasNotify, loadingNotifyCount, openNotifyList } = useNotify()
    ...
    <>
      {isConnected ? (
        <>
          ...
          {hasNotify ? (
            <Button size="medium" orange round={loadingNotifyCount === 0} onClick={openNotifyList}>
              {loadingNotifyCount > 0 ? (
                `${loadingNotifyCount} pending...`
              ) : (
                <svg
