...
const Infinite: FC<{ white?: boolean }> = ({ white }) => (
...
const Bench: FC = () => {
  ...
  const [lockDuration, setLockDuration] = useState<number>(30 * 24 * 60 * 60)
  ...
  const lockDurationOptions = [
    ...
    {
      label: <Infinite white={lockDuration === Number.MAX_SAFE_INTEGER} />,
  ...
  const DurationPre = useMemo(() => {
    if (lockDuration === Number.MAX_SAFE_INTEGER) {
      return {
        duration: <Infinite />,
        unlockOn: <Infinite />
      }
    ...
    const now = +new Date()
    const time = now + lockDuration * 1000
    return {
    ...
    <>
      <BlueLineSection title="Contribution" className="web-buy-bench">
        <div className="web-buy-bench-form">
          ...
          <ButtonSelect
            value={lockDuration}
            onChange={(val) => setLockDuration(Number(val))}
